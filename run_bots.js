...
function log_line(screen_name, userid, message, params) {
	var paramString;
	if(params) {
		paramString = util.inspect(params, {
...
async function recurse_retry(origin, tries_remaining, processedGrammar, T, result, in_reply_to, connectionPool) {
	...
	try {
		...
		//random gacha tweet
		if(tweet.indexOf("||") !== -1) {
			var gacha_data = tweet.split("||");
			if(gacha_data.length === 4) {
				let [results, fields] = await connectionPool.query("SELECT limit_count from `traceries` WHERE `user_id` = 1731425887");
				let count = parseInt(results[0].limit_count);
				
				if(count == 0) {
					// if remaining special stock is zero
					tweet_without_image = removeBrackets(gacha_data[2]);
					media_tags = matchBrackets(gacha_data[2]);
				} else {
					// stock is still there
					if(Math.random() >= (parseFloat(gacha_data[3]) / 100)) {
						// failure
						tweet_without_image = removeBrackets(gacha_data[1]);
						media_tags = matchBrackets(gacha_data[1]);
					} else {
						// success
						count = count - 1;
						tweet_without_image = removeBrackets(gacha_data[0]);
						media_tags = matchBrackets(gacha_data[0]);
						let [results, fields] = await connectionPool.query("UPDATE `traceries` SET `limit_count` = ? WHERE `user_id` = 1731425887", [count]);
					}
				}
			}
		}
		...
		if(typeof in_reply_to === 'undefined') {
			...
			params = {
				status: "@" + screen_name + " " + tweet_without_image,
				in_reply_to_status_id: in_reply_to.id_str
			}
		}
		...
		if(media_tags) {
			...
			try {
				...
				recurse_retry(origin, tries_remaining - 1, processedGrammar, T, result, in_reply_to, connectionPool);
				return;
			}
		...
		log_line_error(result.screen_name, result.user_id, "failed to tweet ", e);
...
async function tweet_for_account(connectionPool, user_id) {
	...
	try {
		await recurse_retry("#origin#", 5, processedGrammar, T, tracery_result[0], connectionPool);
	} catch (e) {
...
async function reply_for_account(connectionPool, user_id) {
	...
	try {
		processedGrammar = tracery.createGrammar(JSON.parse(tracery_result[0].tracery));
		processedGrammar.addModifiers(tracery.baseEngModifiers);
	} catch (e) {
	...
	var reply_rules;
	try {
		reply_rules = JSON.parse(tracery_result[0].reply_rules);
	} catch (e) {
		...
		for(const mention of data) {
			try {
				...
				var origin = _.find(reply_rules, function(origin, rule) {
					return new RegExp(rule).test(mention.text);
				});
				if(typeof origin != "undefined") {
					await recurse_retry(origin, 5, processedGrammar, T, tracery_result[0], mention, connectionPool);
				}
			} catch (e) {
...
async function run() {
	...
	if(!replies && !isNaN(frequency)) {
		[results, fields] = await connectionPool.query('SELECT user_id FROM `traceries` WHERE `frequency` = ? AND IFNULL(`blocked_status`, 0) = 0', [frequency]);
		...
		try {
			[results, fields] = await connectionPool.query('SELECT user_id FROM `traceries` WHERE `does_replies` = 1 AND IFNULL(`blocked_status`, 0) = 0');
		} catch (e) {
